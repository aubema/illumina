...
def coordinate_in_dd_mm_ss(coordinate):
    if coordinate<0:
        ...
        neg=1
    dd = int(coordinate)
    mm = int((float(coordinate) - float(dd)) * 60)
    ss = round(((((float(coordinate) - float(dd)) * 60) - float(mm)) * 60),2)
    return neg, str(dd), str(mm), str(ss)

latneg, lat_deg, lat_min, lat_sec = coordinate_in_dd_mm_ss(domain_params['latitude'])
lonneg, lon_deg, lon_min, lon_sec = coordinate_in_dd_mm_ss(domain_params['longitude'])
