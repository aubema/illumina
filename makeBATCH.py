...
for p in comb(*param_space):
    if "azimuth_angles" in multival:
        ...
        if el == 90 and \
            params['azimuth_angles'].index(
                p[multival.index("azimuth_angles")] ) != 0:
            continue

    fold_name = dir_name + \
        '/'.join([ multival[i]+"_%g" % p[i]
        for i in xrange(len(multival)) ]) + '/'
    ...
    for zon in zones:
        os.symlink(
            ...
            fold_name+params['exp_name']+"_fctem_"+zon+".dat" )
    ...
    for name in ["obstd","obsth","obstf","altlp"]:
        ds = MSD.Open( "%s_%s.hdf5" % \
            ( params['exp_name'], name ) )
        save_bin(fold_name+"%s_%s.bin" % \
            ( params['exp_name'], name ), ds[layer] )

    for zon in zones:
        ds = MSD.Open( "%s_%s_lumlp_%s.hdf5" % \
            ( params['exp_name'], wavelength, zon ) )
        save_bin(fold_name+"%s_lumlp_%s.bin" % \
            ( params['exp_name'], zon ), ds[layer] )

    ds = MSD.Open( "modis_%s.hdf5" % \
        wavelength )
    save_bin(fold_name+"%s_reflect.bin" % \
        params['exp_name'], ds[layer] )
    ...
    with open(fold_name+"illumina.in",'w') as f:
        ...
        f.write("%-*s ! Cell size [m]\n" % \
            (n_space, "%(s)s %(s)s" % \
                {'s':ds.pixel_size(layer)} ) )
        f.write("%-*s ! Aerosol optical cross section file\n" % \
            (n_space, "aerosol.mie.out") )
        f.write("%-*s !\n" % \
            (n_space,'') )
        f.write("%-*s ! "
            "Double scattering radius [m] ; "
            "Scattering step\n" % \
                ( n_space, "%g %d" % (
                    getp("scattering_radius",multival,p,params)*1000,
        ...
        f.write("%-*s ! "
            "500nm aerosol optical depth ; "
            "Angstrom exponent\n" % \
                ( n_space, "%g %g" % (
                    getp("aerosol_optical_depth",multival,p,params),
        ...
        f.write("%-*s ! "
            ...
            "Azimutal viewing angle\n" % \
            ( n_space, "%d %d" % (
                getp("elevation_angles",multival,p,params),
