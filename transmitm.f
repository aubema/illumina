...
c
     ...
     + lambda,dx,dy,pression,transm)
      real angle,deltam,e,pression,transm,lambda                          ! Declaration des variables.
      real lambdam,dist1,dist2,dist1m,dist2m
     ...
     e 10628.87,12840.16,15511.4,18738.26,22636.31,27345.16/
      ...
      lambdam=lambda/1000.                                                ! Les equations suivantes utilisent des lambdas en microns.
       dist1m=3000000.      
       dist2m=3000000.
       do k=1,50 
     ...
     +   then 
            deltam=(exp(-1.*cell_height(zsup)/8000.)*dist)/8000./
     +      sin(angle)
     ...
     +      cos(angle))
      ...
      mprime=deltam*(pression/101.3)
c  transmittance tiree de Kneizys et al. (1980)       
      transm=exp(-1.*mprime/((lambdam**4.)*
     a (115.6406-(1.335/(lambdam**2.)))))
