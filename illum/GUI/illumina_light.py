...
class Ui_ILLUMINA(object):
    def setupUi(self, ILLUMINA):
        ...
        self.label_11.setLocale(
            QtCore.QLocale(QtCore.QLocale.English, QtCore.QLocale.Canada)
        )
    ...
    def retranslateUi(self, ILLUMINA):
        ...
        self.comboBox_atm.setItemText(
            0, _translate("ILLUMINA", "CC Continental clean")
        )
        self.comboBox_atm.setItemText(
            1, _translate("ILLUMINA", "CA Continental average")
        )
        self.comboBox_atm.setItemText(
            2, _translate("ILLUMINA", "CP Continental polluted")
        )
        self.comboBox_atm.setItemText(
            3, _translate("ILLUMINA", "MP Maritime polluted")
        )
        self.comboBox_atm.setItemText(
            4, _translate("ILLUMINA", "MC Maritime clean")
        )
        ...
        self.aerosol_opac_button.setText(
            _translate("ILLUMINA", "Std values (OPAC)")
        )
        ...
        self.groupBox_6.setTitle(
            _translate(
                "ILLUMINA", "5. Architectural features around light sources"
            )
        )
        self.hlamp_edit.setText(_translate("ILLUMINA", "0"))
        self.label_7.setText(
            _translate("ILLUMINA", "Mean heigh of all the lamps (m)")
        )
        self.dobs_edit.setText(_translate("ILLUMINA", "0"))
        self.label_8.setText(_translate("ILLUMINA", "Mean street width (m)"))
        self.label_18.setText(
            _translate("ILLUMINA", "Mean building story number")
        )
        self.hobs_edit.setText(_translate("ILLUMINA", "0"))
        self.obstacles_checkbox.setText(_translate("ILLUMINA", "No obstacles"))
        self.exp_def_button.setText(
            _translate("ILLUMINA", "6. Define simulation")
        )
        self.Run_simulation.setText(
            _translate("ILLUMINA", "7. Run Simulation")
        )
    ...
    def defining_exp(self):
        ...
        # creating inventory
        a1 = "\t".join([lat, long, radius, hobs, dobs, fobs, hlamp]) + "\t"
        a = []
        ...
        for f in files:
            os.remove(f)
        name_viirs = glob.glob(
            "VIIRS_database/SVDNB_npp_2020" + date_month + "*"
        )
        ...
        with open("inputs_params.in", "w") as f:
            for line in lines:
                if line[:15] == "aerosol_profile":
                    f.write(
                        "aerosol_profile: " + atm_type.split(" ")[0] + "\n"
                    )
        ...
        if self.obstacles_checkbox.isChecked():
            ...
            log_4 = (
                ...
                + " Obstacle heigh: "
                + hobs
                + " Distance between lamps and obstacles: "
                + dobs
                + "\n"
            )
        ...
        for i in range(0, len(perc)):
            ...
            log_6 = (
                log_6 + perc2[i] + "% " + tech[i] + " Ulor:" + ulor[i] + "\n"
            )
    ...
    def run_simulation(self):
        ...
        while job_cont < jobs:
            for wl in wavel:
                for ly in layer:
                    time.sleep(0.5)
                    path = (
                        "./exec/elevation_angle_90/azimuth_angle_0/wavelength_"
                        + str(wl)
                        + "/layer_"
                        + ly
                    )
                    if os.path.isfile(path + "/finished.txt") is True:
                        job_cont += 1
        ...
        sens_filt = (
            sens[index_min[0][0] : index_max[0][0]]
        ) / 100  # sensitivity in file is as %
        ...
        log1 = (
            "Artificial zenith sky brightness: "
            + str(round(mag_V_art, 2))
            + " mag/arcsec\n"
        )
        if self.natural_light.isChecked():
            ...
            log2 = (
                "Natural zenith sky brightness: "
                + str(round(mag_V_nat, 2))
                + " mag/arcsec\n"
            )
            log3 = (
                "Total zenith sky brightness: "
                + str(round(mag_V_tot, 2))
                + " mag/arcsec\n"
            )
            log1 = log1 + log2 + log3
        else:
        ...
        for i in range(0, len(perc)):
            log_6 = (
                log_6 + perc[i] + "% " + tech[i] + " Ulor:" + ulor[i] + "\n"
            )
        ...
        dict_file = (
            ...
            + " (m)\n"
            + "Aerosol type: "
            + self.comboBox_atm.currentText().split(" ")[0]
            + "\n"
            + "Relative humidity: "
            + self.comboBox_rh.currentText()
            + "\n"
            + "Total radiance: "
            + str(round(mag_V_tot, 2))
            + " mag/arcsec\n"
        ...
        # aeh = self.box_aeh.text()
        with open(
            "Results_experiment_" + self.name_edit.text() + ".txt", "w"
        ) as file:
