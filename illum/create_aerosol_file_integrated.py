...
with open("./Inputs/integration_limits.dat",'r') as fwavelenght:
    ...
    bins=int(lines[0])
aerosol_types = ['inso','waso','soot','ssam','sscm','minm','miam','micm','mitr','suso','fogr']
contlayer=0
for combination_type in layer:
    N = np.array([0] * len(aerosol_types), dtype='float')
    for i in range(len(aerosol_types)):
        N[i]=0
    if combination_type=='Manual' or combination_type=='MANUAL' or combination_type=='manual':
        if contlayer==0:
    ...
    for i in range(1, bins + 1):
        ...
        for type in aerosol_types:
            if type== 'waso' or type == 'ssam' or type == 'sscm' or type=='suso':
                RH2=str(RH)
            else:
                RH2='00'
            with open(mie_path+"/OPAC_data/"+type+RH2, 'r') as finso:
                ...
                for i, line in enumerate(finso):
                    ...
                    if i >= 86:
                        ...
                        contwl = 0
                        for aa in temp2:
                            pf_array[contwl][i - 86] = float(aa)  # pf for a wl and a scat angle
                            contwl = contwl + 1

            ext_type[j] = np.interp(wl / 1000, wl_prop[:, 0], wl_prop[:, 1])
            scat_type[j] = np.interp(wl / 1000, wl_prop[:, 0], wl_prop[:, 2])
            pf_function = interpolate.interp2d(scat_angle, wl_prop[:, 0], pf_array, kind='cubic') #cubic interpolation for a given wl and angle
            for i in range(181):
                pf_type[i,j]=pf_function(i,wl/1000) #pf function i=angle, j=type
            j=j+1
        ...
        for i in range(181):
            ...
            norm_factor = norm_factor + (1 / (4 * math.pi)) * 2 * math.pi * pf_raw[i, 1] * math.sin(
                pf_raw[i, 0]) * math.pi / 180
        with open('./Inputs/' + combination_type + '_' + str(int(wl)) + '.txt', 'w+') as f:
