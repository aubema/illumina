...
c
     ...
     +  cloudz,lcib,ncib,scalef) 
        do i=1,ncib                           
          ...
          jj=lcib(i,2)
          zz=altsol(ii,jj)
          if (altsol(ii,jj).gt.altsol(x,y)) then
            zhoriz=pi/2.-atan((altsol(ii,jj)-altsol(x,y))/sqrt(dx**
     +      2.*real((ii-x))**2.+dy**2.*real((jj-y))**2.))
            d=sqrt(dx**2.*real((ii-x))**2.+dy**2.*real((jj-y))**2.)
          else
