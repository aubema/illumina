...
c
       ...
       real slope569,slope615,wt615,wt569,FWHM,ele,azi,l
       integer i,j
...
c response of the V filter
       wt615=exp(-(615.-551.)**2./2./(FWHM/2.35)**2.)
       wt569=exp(-(569.-551.)**2./2./(FWHM/2.35)**2.)
c       print*,wt615,wt569
c       print*,exp(-(489.-551.)**2./2./(FWHM/2.35)**2.)
c Considering that most of the flux is coming from AND589nm, we used 
c 569 and 615 Na lines
c we assume that mnz is constant all over the sky
       mn=mnz
       open(unit=1,file='input615.txt',status='unknown')
       do i=1,25
         read(1,*) el1(i),az1(i),la1(i)
       enddo
       close(unit=1)
       open(unit=1,file='after615.txt',status='unknown')
       do i=1,25
         read(1,*) ele,azi,l
         if (ele.eq.90.) laz1=l
       enddo
       ...
       enddo
       close(unit=1) 
       laz=laz1*slope615*wt615+laz2*slope569*wt569
       ...
       do i=1,25
        do j=1,25
         if ((az2(j).eq.az1(i)).and.(el2(j).eq.el1(i))) then
           az(i)=az1(i)
           el(i)=el1(i)
           la(i)=la1(i)*slope615*wt615+la2(j)*slope569*wt569
